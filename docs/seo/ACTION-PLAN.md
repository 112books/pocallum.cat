...
### C1 · Crear i verificar Google Business Profile
...
### C2 · Corregir les capçaleres dels serveis trencades a `/serveis/`
...
### C3 · Fer el hero de portada server-rendered (LCP)
...
### A1 · Afegir "Barcelona" al H1 i title de `/serveis/`
...
### A2 · Afegir meta descriptions a totes les pàgines principals
...
### A3 · Corregir `og:type` per a pàgines de secció
...
### A4 · Corregir hreflang `x-default` a les pàgines EN
...
### A5 · Corregir double-encoding al BreadcrumbList
...
### A6 · Afegir `translationKey` a les pàgines de serveis CA/EN
...
### A7 · Afegir el telèfon visible al contingut de `/contacte/`
...
### A8 · Corregir dates futures al sitemap (festivals)
...
### A9 · Publicar 3–5 testimonis de clients
...
### A10 · Corregir `og:image` fallback per a pàgines de secció
...
### M1 · Crear 3 landing pages amb keyword targeting
...
Prioritat:
1. `/serveis/concerts-barcelona/` — H1: "Fotografia de concerts i directes a Barcelona"
2. `/serveis/teatre-dansa-barcelona/` — H1: "Fotografia de teatre i dansa a Barcelona"
3. `/en/concert-photographer-barcelona/` — H1: "Concert Photographer Barcelona"

Cada landing: 400+ paraules, FAQ (4 preguntes), schema `Service`, CTA únic, 6–8 imatges inline.

### M2 · Ampliar el cos de text de les pàgines de serveis a 140–160 paraules
...
### M5 · Afegir schema `LocalBusiness` a `/contacte/` i `/serveis/`
...
### M6 · Corregir el tipus de schema als festivals no musicals
...
### M8 · Afegir bloc "Sobre l'autor" a les notícies
...
### M9 · Moure `__heroData` inline script al final del `<body>`
...
### M10 · Activar `disableHugoGeneratorInject = true` a `hugo.toml`
...
### B1 · IndexNow — notificació immediata d'indexació
Afegir fitxer clau a `static/` + step a GitHub Actions que pinga l'API en cada deploy.

### B2 · Eliminar `<changefreq>` i `<priority>` del sitemap
...
### B4 · Corregir el `<link rel="shortcut icon">` deprecated
Eliminar la línia deprecated de `head.html`. Mantenir només `rel="icon"`.

### B5 · Canal YouTube amb 3–5 vídeos curts
...
### B6 · Uniformitzar les dates a `llms.txt`
CA diu "des del 2010" (marca), EN diu "active since 2002" (Joan com a fotògraf). Aclarir ambdós.

### B7 · Verificar i corregir coordenades geo al schema
...
### B8 · Facebook a `sameAs` del schema
